...
void CUser::GameStart(Packet & pkt)
{
	...
	if (opcode == 1)
	{
		...
		if (g_pMain->pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR
			&& !g_pMain->pTempleEvent.isActive)
		{
			uint32_t elapsedTime = (uint32_t)UNIXTIME - g_pMain->pTempleEvent.StartTime;
			uint16_t remainingTime = 0;

			if (elapsedTime < 600)
				remainingTime = (uint16_t)(600 - elapsedTime);

			Packet eventPacket(WIZ_EVENT);

			eventPacket << uint8_t(TEMPLE_EVENT);
			eventPacket << uint16_t(TEMPLE_EVENT_BORDER_DEFENCE_WAR);
			eventPacket << remainingTime;
