...
*/
void CGameServerDlg::Send_Zone_Matched_Class(
	Packet* pkt,
	uint8_t bZoneID,
	CUser* pExceptUser,
	uint8_t nation,
	uint8_t seekingPartyOptions)
{
	SessionMap sessMap = g_pMain->m_socketMgr.GetActiveSessionMap();
	foreach(itr, sessMap)
	{
...
void CGameServerDlg::Send_Region(
	Packet* pkt,
	C3DMap* pMap,
	int x,
	int z,
...
void CGameServerDlg::Send_UnitRegion(
	Packet* pkt,
	C3DMap* pMap,
	int x,
	int z,
...
void CGameServerDlg::Send_OldRegions(
	Packet *pkt,
	int old_x,
	int old_z,
	C3DMap *pMap,
	...
	CUser* pExceptUser)
{
	if (old_x != 0)
	{
		Send_UnitRegion(pkt, pMap, x + old_x*2, z + old_z - 1, nullptr);
		Send_UnitRegion(pkt, pMap, x + old_x*2, z + old_z, nullptr);
		Send_UnitRegion(pkt, pMap, x + old_x*2, z + old_z + 1, nullptr);
	}

	if (old_z != 0)
	{
		Send_UnitRegion(pkt, pMap, x + old_x, z + old_z*2, nullptr);
		if (old_x < 0)
			Send_UnitRegion(pkt, pMap, x + old_x + 1, z + old_z*2, nullptr);
		else if (old_x > 0)
			Send_UnitRegion(pkt, pMap, x + old_x - 1, z + old_z*2, nullptr);
		else
		{
...
void CGameServerDlg::Send_NewRegions(
	Packet* pkt,
	int new_x,
	int new_z,
	C3DMap* pMap,
	...
	CUser* pExceptUser)
{
	if (new_x != 0)
	{
		Send_UnitRegion(pkt, pMap, x + new_x, z - 1, nullptr);
		Send_UnitRegion(pkt, pMap, x + new_x, z, nullptr);
		Send_UnitRegion(pkt, pMap, x + new_x, z + 1, nullptr);
	}
	...
	if (new_z != 0)
	{
		...
		if (new_x < 0)
			Send_UnitRegion(pkt, pMap, x + 1, z + new_z, nullptr);
		else if (new_x > 0)
			Send_UnitRegion(pkt, pMap, x - 1, z + new_z, nullptr);
		else
		{
...
void CGameServerDlg::Send_NearRegion(
	Packet* pkt,
	C3DMap* pMap,
	int region_x,
	int region_z,
...
void CGameServerDlg::Send_FilterUnitRegion(
	Packet* pkt,
	C3DMap* pMap,
	int x,
	int z,
...
void CGameServerDlg::GetRegionUserIn(
	C3DMap* pMap,
	uint16_t region_x,
	uint16_t region_z,
	Packet& pkt,
...
void CGameServerDlg::GetRegionUserList(
	C3DMap* pMap,
	uint16_t region_x,
	uint16_t region_z,
	Packet& pkt,
...
void CGameServerDlg::GetRegionMerchantUserIn(
	C3DMap* pMap,
	uint16_t region_x,
	uint16_t region_z,
	Packet& pkt,
...
void CGameServerDlg::NpcInOutForMe(CUser* pSendUser)
{
	...
	int16_t rx = pSendUser->GetRegionX(), rz = pSendUser->GetRegionZ();
	foreach_region(x, z)
		GetRegionNpcIn(pMap, rx + x, rz + z, result, npc_count, pSendUser);

	result.put(wpos, npc_count);
	pSendUser->SendCompressed(&result);
}

void CGameServerDlg::GetRegionNpcIn(
	C3DMap* pMap,
	uint16_t region_x,
	uint16_t region_z,
	Packet& pkt,
...
void CGameServerDlg::GetRegionNpcList(
	C3DMap* pMap,
	uint16_t region_x,
	uint16_t region_z,
	Packet& pkt,
...
void CGameServerDlg::TempleEventTimer()
{
	uint32_t nHour = g_localTime.tm_hour;
	uint32_t nMinute = g_localTime.tm_min;
	uint32_t nSeconds = g_localTime.tm_sec;
...
void CGameServerDlg::TempleEventStart()
{
		...
		&& !pTempleEvent.isActive)
	{
		...
		std::string bufferpro = string_format(
			"[Event Message] (%d~%d) levels can register for Border Defance War.",
			pTempleEvent.m_nBorderDefenseWarMINLEVEL,
			pTempleEvent.m_nBorderDefenseWarMAXLEVEL
		);

		ChatPacket::Construct(&resultmer, 7, &bufferpro);
		Send_All(&resultmer);

		SendAnnouncement(bufferpro.c_str());

		/*
			BDW REGISTER SCREEN

			Client tarafındaki HandleEventProcess():
		...
		*/
		...
		Packet eventPacket(WIZ_EVENT);

		eventPacket << uint8_t(TEMPLE_EVENT);
		eventPacket << uint16_t(TEMPLE_EVENT_BORDER_DEFENCE_WAR);
		eventPacket << uint16_t(600);

		Send_All(&eventPacket);
	}
		...
		&& pTempleEvent.isActive)
	{
		...
		std::string bufferpro =
			"[Event Message] Border Defance War has started.";

		ChatPacket::Construct(&resultmer2, 7, &bufferpro);
		Send_All(&resultmer2);
...
{
	uint16_t kcount = g_pMain->pTempleEvent.KarusUserCount;
	uint16_t ecount = g_pMain->pTempleEvent.ElMoradUserCount;

	Packet result(WIZ_EVENT);

	result << uint8_t(TEMPLE_EVENT_COUNTER);
	result << uint16_t(TEMPLE_EVENT_BORDER_DEFENCE_WAR);
	result << kcount;
	result << ecount;

	printf(
		"[BDW] COUNTER SEND | Karus=%u Human=%u\n",
		kcount,
		ecount
	);
	// Sadece belirli oyuncuya gönder
	if (pUser != nullptr)
	{
		pUser->Send(&result);
		return;
	...
	// Event'e kayıtlı tüm oyunculara gönder
	// m_TempleEventUserArray socket ID ile tutuluyor.
	for (int i = 0; i < MAX_USER; i++)
	{
		_TEMPLE_EVENT_USER* pEventUser =
			g_pMain->m_TempleEventUserArray.GetData(i);

		if (pEventUser == nullptr)
			continue;

		CUser* pSendUser = g_pMain->GetUserPtr(pEventUser->m_socketID);

		if (pSendUser == nullptr)
			continue;

		if (!pSendUser->isInGame())
			continue;

		if (!pSendUser->isEventUser())
			continue;
...
void CGameServerDlg::TempleEventFinish(uint16_t nEventRoom)
{
...
void CGameServerDlg::TempleEventKickOutUser(uint16_t nEventRoom)
{
	uint8_t nZoneID = 0;
...
void CGameServerDlg::TerminationFinish(uint16_t nEventRoom)
{
	if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
	{
		...
		bresult << uint8_t(10)
			<< uint8_t(2) << uint8_t(0)
			<< uint8_t(0) << uint8_t(20) << uint32_t(0);

		Packet cresult(WIZ_EVENT);
		cresult << uint8_t(10)
			<< uint8_t(2) << uint8_t(0)
			<< uint8_t(1) << uint8_t(20) << uint32_t(0);

		Packet dresult(WIZ_EVENT);
		dresult << uint8_t(10)
			<< uint8_t(1) << uint8_t(0)
			<< uint8_t(2) << uint8_t(20) << uint32_t(0);
