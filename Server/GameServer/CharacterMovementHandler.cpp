...
{
	if (g_pMain->pTempleEvent.ActiveEvent != TEMPLE_EVENT_BORDER_DEFENCE_WAR
		|| isEventUser())
		return;

	C3DMap* pMap =
		g_pMain->GetZoneByID(ZONE_BORDER_DEFENSE_WAR);
	...
	if (!CanChangeZone(pMap, errorReason))
	{
		Packet hata(WIZ_WARP_LIST, uint8_t(2));

		hata << uint8_t(errorReason);
	...
	// =====================================================
	// GÜNCEL BDW SAYILARINI CLIENT'E GÖNDER
	// =====================================================

	Packet result(WIZ_EVENT);

	result << uint8_t(TEMPLE_EVENT_BORDER_DEFENCE_WAR);
	result << uint16_t(g_pMain->pTempleEvent.KarusUserCount);
	result << uint16_t(g_pMain->pTempleEvent.ElMoradUserCount);
	result << uint16_t(g_pMain->m_nTempleEventRemainSeconds);
	...
	std::string bufferpro = string_format(
		"[Event Message] Border Defance War Human: %d, Karus: %d, Remaining time: %d sec",
		g_pMain->pTempleEvent.ElMoradUserCount,
		g_pMain->pTempleEvent.KarusUserCount,
		g_pMain->m_nTempleEventRemainSeconds
	);

	ChatPacket::Construct(
		&resultmer,
		7,
		&bufferpro
	);

	if ((g_pMain->pTempleEvent.KarusUserCount +
		g_pMain->pTempleEvent.ElMoradUserCount) % 5 == 0)
	{
		g_pMain->Send_All(
			&resultmer,
			nullptr,
